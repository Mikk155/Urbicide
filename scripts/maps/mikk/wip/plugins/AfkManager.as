...
void MapStart()
{
    ...
    if( ListOfPlugins.find( "multi_language" ) >= 0)
    {
        ...
        RIPENT::LoadRipentFile( "scripts/plugins/mikk/AfkManager/PluginMessages.ent" );
    }
    else
    {
...
class CBaseAfkManagerZone : ScriptBaseEntity, UTILS::MoreKeyValues
{
    ...
    void TriggerThink() 
    {
        ...
        for( int iPlayer = 1; iPlayer <= g_PlayerFuncs.GetNumPlayers(); ++iPlayer )
        {
            ...
            if( self.pev.SpawnFlagBitSet( 2 ) || UTILS::InsideZone( pPlayer, self ) )
                UTILS::SetFloatCKV( pPlayer, "$i_afktimer", 0 );
        }
        self.pev.nextthink = g_Engine.time + 0.2f;
    }
}
...
void AFKThink()
{
    for( int iPlayer = 1; iPlayer <= g_Engine.maxClients; ++iPlayer )
    {
        ...
        int iafktimer = UTILS::GetFloatCKV( pPlayer, "$i_afktimer" );

        if( pPlayer.IsAlive() && !pPlayer.IsMoving() )
        {
            UTILS::SetFloatCKV( pPlayer, "$i_afktimer", iafktimer - 1 );
        ...
        {
            UTILS::SetFloatCKV( pPlayer, "$i_afktimer", AFKMaxTime );
        }

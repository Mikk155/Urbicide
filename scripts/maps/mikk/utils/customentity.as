...
mixin class ScriptBaseCustomEntity
{
    ...
    bool IsLockedByMaster( CBaseEntity@ &in pActivator = null, USE_TYPE &in UseType = USE_TOGGLE )
    {
        if( !m_iszMaster.IsEmpty() and !g_EntityFuncs.IsMasterTriggered( m_iszMaster, self ) )
        {
            ...
            if( iszTarget != '' )
            {
                ...
                g_Util.Trigger( iszTarget, ( pActivator !is null ) ? pActivator : null, pCaller, UseType, 0.0f );
            }
            return true;
        }
        return false;
